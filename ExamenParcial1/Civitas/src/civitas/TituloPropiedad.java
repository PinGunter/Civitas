...
public class TituloPropiedad {
    ...
    // EXAMEN
    TituloPropiedad(String nombre) {
        tipo = TipoPropiedad.ESTACION;
        this.nombre = nombre;
        alquilerBase = -1;
        factorRevalorizacion = -1;
        hipotecaBase = -1;
        precioCompra = -1;
        precioEdificar = -1;
        propietario = null;
        numCasas = numHoteles = -1;
        hipotecado = false;
    ...
    boolean construirCasa(Jugador jugador) {
        ...
        if (tipo == TipoPropiedad.SOLAR) {
            ...
            return result;
        } else if (tipo == TipoPropiedad.ESTACION) {
            Diario.getInstance().ocurreEvento("ERROR: Se intenta construir casa en estacion. Se penaliza al jugador");
            // como el que llama a construir casa es el propietario, comprobamos que lo sea y lo ponemos a null como penalizacion
            // eliminamos del array de propiedades la propiedad
            if (esEsteElPropietario(jugador)) {
                propietario = null;
