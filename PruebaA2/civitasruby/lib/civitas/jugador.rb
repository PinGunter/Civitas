...
#      puts "ip: #{ip}"
#      puts "size: #{@propiedades.size}"
#      puts "ip >= 0 #{(ip>=0).to_s}"
#      puts "ip < @propiedades.size #{(ip<@propiedades.size).to_s}"
...
#      puts "@@@@@@@@\nLlamada hacer_sostenible jugador\n@@@@@@@@@@@@@@@"
      propiedad_sostenible = nil
      puts @encarcelado.to_s
      puts existe_la_propiedad(ip)
      puts !@encarcelado && existe_la_propiedad(ip)
      if !@encarcelado && existe_la_propiedad(ip)
        propiedad = @propiedades.at(ip)
        cantidad = propiedad.get_importe_hacerme_sostenible
        puedo_gastar = puedo_gastar(cantidad)
        if (puedo_gastar)
         propiedad_sostenible = propiedad.hacerme_sostenible(self)
         if (!propiedad_sostenible.nil?)
           @propiedades[ip] = propiedad_sostenible
           Diario.instance.ocurre_evento("#{@nombre} tiene ahora la propiedad sostenible #{propiedad_sostenible.get_nombre}")
         end
        end
      end 
#      puts "Propiedadad sostenible #{propiedad_sostenible.get_nombre}"
