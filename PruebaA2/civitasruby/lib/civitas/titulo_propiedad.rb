  ...
  class Titulo_propiedad
    ...
    def to_s()
      "\n-TituloPropiedad: \nNombre: #{@nombre} \nHipotecada: #{@hipotecada}
         ...
         \nPorcentaje inversion sostenible : #{calcular_porcentaje_inversion_por_sostenibilidad}\n"
    end
    ...
    def calcular_porcentaje_inversion_por_sostenibilidad
      if get_num_casas == 0
        return 0.05
      else
        return 0.02
    ...
    def get_importe_hacerme_sostenible
      get_precio_venta*(1+calcular_porcentaje_inversion_por_sostenibilidad)
    end
    
    def hacerme_sostenible(jugador)
      propiedad_sostenible = nil
      es_el_propietario = es_este_el_propietario(jugador)
      if es_el_propietario
        puts "#{jugador.get_nombre} es el propietario"
        percent = calcular_porcentaje_inversion_por_sostenibilidad
        importe = get_importe_hacerme_sostenible
        jugador.paga(importe)
        propiedad_sostenible = Propiedad_ecologica.new(percent, @nombre, @alquiler_base, @factor_revalorizacion, @hipoteca_base, @precio_compra, @precio_edificar)
      end
      return propiedad_sostenible
    end
